  ...
  <div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="k in keys" :key="k.id">
          <td>{{ k.name }}</td>
          <td>{{ k.provider }}</td>
          <td style="color:#888;font-size:12px">{{ k.api_key_preview }}</td>
          <td>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="closeForm">
      <form class="modal" @submit.prevent="submit" style="min-width:400px">
        ...
        <input v-model="form.api_key" :placeholder="editId ? '留空则不修改 Key' : 'API Key'" type="password" :required="!editId" />
...
function edit(k) { editId.value = k.id; form.name = k.name; form.provider = k.provider; form.api_key = ''; showForm.value = true; }
function closeForm() { showForm.value = false; editId.value = null; form.name = ''; form.provider = ''; form.api_key = ''; }

async function submit() {
  loading.value = true; error.value = '';
  try {
    const body = { name: form.name, provider: form.provider };
    if (form.api_key.trim()) body.api_key = form.api_key.trim();
    if (editId.value) {
      await api.patch(`/admin/api-keys/${editId.value}`, body);
    } else {
      if (!body.api_key) { error.value = '请填写 API Key'; return; }
